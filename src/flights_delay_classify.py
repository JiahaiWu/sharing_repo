...
def load_data(data_file='../data/flights.csv', target_col=target_col, rand_seed=rand_seed):
    ...
    data.drop('flight_id', axis=1, inplace=True)
...
def KFold_EM(X_train, X_test, y_train, y_test, cate_cols, clf):
    cate_cols_indexs = np.where(X_train.columns.isin(cate_cols))[0]  
    folds = StratifiedKFold(n_splits=5, shuffle=True, random_state=rand_seed)
